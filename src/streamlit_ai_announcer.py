...
# 절대 경로로 직접 설정 (Streamlit 환경에서 상대 경로 문제 해결)
data_dir = r'C:\Users\samga\Documents\GitHub\Ai_anchor\data'
models_dir = r'C:\Users\samga\Documents\GitHub\Ai_anchor\models'

# 모델 관련 imports
try:
    import tensorflow as tf
    tf.get_logger().setLevel('ERROR')  # TensorFlow 경고 숨기기
...
MODELS_AVAILABLE = TENSORFLOW_AVAILABLE and OPENAI_AVAILABLE
...
def load_match_data():
    ...
    try:
        csv_path = os.path.join(data_dir, 'lol_for_user.csv')
...
def load_lstm_model():
    ...
    try:
        model_path = os.path.join(models_dir, 'best_lol_lstm_model.h5')
